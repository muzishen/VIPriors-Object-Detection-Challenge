...
INPUT_DIR = r'D:\xf\xray\xray\all_data\save\images\powerbank'
OUTPUT_DIR = 'output'
AUGMENT_SIZE = 1
...
def main():
    for file in glob.glob('%s/*.xml' % INPUT_DIR):
        ...
        augment(annotation, file.split('\\')[-1].replace('xml', 'jpg'))

    # for file in glob.glob('%s/*.xml' % OUTPUT_DIR):
    #     an.inspect(file)


def augment(annotation, filename):
    seq = sequence.get()
    ...
    for i in range(AUGMENT_SIZE):
        ...
        image = cv2.imread('%s/%s'% (INPUT_DIR, filename))
        _bbs = []
